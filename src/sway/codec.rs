...
impl Decoder for SwayPacketCodec {
    ...
    fn decode(
        ...
        if src[0..super::MAGIC_LEN] != super::MAGIC {
            src.clear();
...
impl Encoder<SwayPacketRaw> for SwayPacketCodec {
    ...
    fn encode(
        ...
        dst.extend_from_slice(&super::MAGIC);
        dst.put_u32_ne(payload_len as u32);
        dst.put_u32_ne(item.packet_type);
...
mod tests {
    ...
    async fn decode_normal() {
        ...
        let mock = Builder::new()
            .read(b"i3-ipc")
            .read(&(payload.len() as u32).to_ne_bytes())
            .read(&(payload_type as u32).to_ne_bytes())
            .read(payload)
            .read(b"i3-ipc")
            .read(&(payload2.len() as u32).to_ne_bytes())
            .read(&(payload2_type as u32).to_ne_bytes())
            .read(payload2)
            .build();
    ...
    async fn encode() {
        ...
        let payload_type = 10;
        let payload2: &[u8] = b"something_else";
        let payload2_type = 101;

        let mock = Builder::new()
            .write(b"i3-ipc")
            .write(&(payload.len() as u32).to_ne_bytes())
            .write(&(payload_type as u32).to_ne_bytes())
            .write(payload)
            .write(b"i3-ipc")
            .write(&(payload2.len() as u32).to_ne_bytes())
            .write(&(payload2_type as u32).to_ne_bytes())
            .write(payload2)
            .build();
